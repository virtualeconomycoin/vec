...
#if defined(NDEBUG)
# error "Virtualeconomycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1fc3d9142f99bf0c528bd81bbe09db953120f5b7b57efe6a61515b542e409de1");
...
const string strMessageMagic = "Virtualeconomycoin Signed Message:\n";
...
{
    // Virtualeconomycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Virtualeconomycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // Virtualeconomycoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Virtualeconomycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Virtualeconomycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Virtualeconomycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Virtualeconomycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x623a83f355df00d62e28eae0b1b83db00ce880c0f1465a6acd1763002d101113");
    }
...
printf("special comment");
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "eksisozluk 25/Aug/2014 ikea'da begendigi mobilyayi marangoza yaptiran tip";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1409093028;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 393855975;

        if (fTestNet)
        {
            block.nTime    = 1409067446;
            block.nNonce   = 386078564;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xaa96d27f88617e14c6d9e58656fcecb482074c32afbbb43ea38db504bba3b030"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Virtualeconomycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("VirtualeconomycoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VirtualeconomycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VirtualeconomycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VirtualeconomycoinMiner(CWallet *pwallet)
{
    printf("VirtualeconomycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("virtualeconomycoin-miner");
    ...
    try { loop {
        ...
        printf("Running VirtualeconomycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VirtualeconomycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VirtualeconomycoinMiner, pwallet));
}
